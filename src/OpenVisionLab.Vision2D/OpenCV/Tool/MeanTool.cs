        ...
        private void AddMeanResult(Mat imageMean, OpenCvSharp.Rect resultBounds, int index)
        {
            switch (property.MEAN_TYPES)
            {
                case MeanType.Mean:
                    ...
                    results.Add(new MeanResult(index, meanValue, OpenVisionLab.Core.CommonConverter.RectToRectangle(resultBounds)));
                    break;
                case MeanType.MeanStdDev:
                    Cv2.MeanStdDev(imageMean, out _, out Scalar stddev);
                    double meanStdDev = Math.Round(stddev[0], 1);
                    results.Add(new MeanResult(index, meanStdDev, OpenVisionLab.Core.CommonConverter.RectToRectangle(resultBounds)));
                    break;
            }
        }
    }
