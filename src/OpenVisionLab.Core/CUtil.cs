...
namespace OpenVisionLab.Core
{
    public class CUtil
    {
        ...
        public static void SynchFolder(DirectoryInfo existingDir, DirectoryInfo copyDir)
        {
                    ...
                    foreach (FileInfo copyFile in copyFiles)
                    {
                        ...
                        if (existingFile.Name == copyFile.Name)
                        {
                            ...
                            if (existingFile.LastWriteTime != copyFile.LastWriteTime)
                            {
                                ...
                                {
                                    Console.Error.WriteLine(desc.Message);
                                }
            ...
            {
                Console.Error.WriteLine($"[FAILED] {nameof(CUtil)}==>{nameof(SynchFolder)} Exception ==> {desc.Message}");
            }
        }
        ...
        public static bool InitDirectory(string strFolderName)
        {
            ...
            {
                Console.Error.WriteLine($"[FAILED] {nameof(CUtil)}==>{nameof(InitDirectory)} Exception ==> {desc.Message}");
                return false;
            }
        }
